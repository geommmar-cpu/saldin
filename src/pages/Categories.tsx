...
export default function Categories() {
  ...
  const [newColor, setNewColor] = useState("text-muted-foreground");
  ...
  const openCreateDialog = () => {
    ...
    setNewName("");
    setNewColor("text-muted-foreground");
    setDialogOpen(true);
  };

  const openEditDialog = (cat: { id: string; name: string; color: string | null }) => {
    setEditingCategory({ id: cat.id, name: cat.name, color: cat.color || "text-muted-foreground" });
    setNewName(cat.name);
    setNewColor(cat.color || "text-muted-foreground");
    setDialogOpen(true);
  };
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {customCategories.map((cat, index) => (
                  ...
                  >
                    <div className={`w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center ${cat.color || "text-primary"}`}>
                  ...
                  {colorOptions.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value}>
                      <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 rounded-full ${opt.value.replace("text-", "bg-")}`} />
                        {opt.label}
                      </div>
                    </SelectItem>
                  ))}
